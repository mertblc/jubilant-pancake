...
def setup_database_connection():
    ...
    try:
        ...
        with open('chess_db_app/sql/create_tables.sql', 'r') as f:
            sql_script = f.read()
    ...
    try:
        ...
        try:
                ...
                for _, row in df.iterrows():
                    try:
                        ...
                        """, (
                            ...
                            row['surname'],
                            row['nationality'],
                            str(row['date_of_birth']),  # Keep original format
                            row['fide_id'],
                            row['elo_rating'],
                            row['title_id']
                        ))
            ...
            if 'CoachCertifications' in xls.sheet_names:
                logging.info("Importing CoachCertifications...")
                df = pd.read_excel(excel_file, sheet_name='CoachCertifications')
                for _, row in df.iterrows():
                    coach_id = get_user_id(cursor, row['coach_username'], 'coach')
                    if coach_id:
                        cursor.execute("""
                            INSERT INTO coach_certifications (coach_id, certification)
                            VALUES (%s, %s)
                        """, (coach_id, row['certification']))
            ...
            if 'ArbiterCertifications' in xls.sheet_names:
                logging.info("Importing ArbiterCertifications...")
                df = pd.read_excel(excel_file, sheet_name='ArbiterCertifications')
                for _, row in df.iterrows():
                    try:
                        # Get arbiter_id and clear any previous results
                        cursor.execute("SELECT user_id FROM users WHERE username = %s", (row['username'],))
                        result = cursor.fetchone()
                        cursor.fetchall()  # Clear any remaining results
                        ...
                        if result:
                            arbiter_id = result[0]
                            cursor.execute("""
                                INSERT INTO arbiter_certifications (arbiter_id, certification)
                                VALUES (%s, %s)
                            """, (arbiter_id, row['certification']))
                            cursor.fetchall()  # Clear any remaining results
                            logging.info(f"Imported certification for arbiter {row['username']}")
                        else:
                            logging.warning(f"Could not find arbiter_id for {row['username']}, skipping")
                    except mysql.connector.Error as err:
                        logging.warning(f"Warning importing arbiter certification for {row['username']}: {err}")
                        cursor.fetchall()  # Clear any remaining results
                        continue
            ...
            if 'Matches' in xls.sheet_names:
                ...
                for _, row in df.iterrows():
                    try:
                        ...
                        if arbiter_id:
                            match_date = convert_date_to_ddmmyyyy(row['date'])
                            cursor.execute("""
            ...
            if 'MatchAssignments' in xls.sheet_names:
                ...
                for _, row in df.iterrows():
                    ...
                    if white_player_id and black_player_id:
                        # Get team IDs
                        cursor.execute("""
                            SELECT team_id FROM player_team_membership 
                            WHERE player_id = %s
                        """, (white_player_id,))
                        result = cursor.fetchone()
                        if result:
                            ...
                            logging.warning(f"No team found for player {white_player_id}")
                            continue
                        ...
                        if result:
                            ...
                            logging.warning(f"No team found for player {black_player_id}")
                            continue
                        ...
                        cursor.execute("""
                            ...
                            VALUES (%s, %s, %s), (%s, %s, %s)
                        """, (row['match_id'], team1_id, white_player_id,
                            row['match_id'], team2_id, black_player_id))
                        ...
                        cursor.execute("""
                            ...
                            SET result = %s 
                            WHERE match_id = %s
                        """, (row['result'], row['match_id']))
            
            conn.commit()
            logging.info("Initial data import completed successfully!")
            ...
            cursor.execute("""
                ...
                BEGIN
                    IF EXISTS (
                        ...
                        AND (
                            (NEW.end_date IS NULL AND start_date >= NEW.start_date)
                            OR (end_date IS NULL AND NEW.start_date >= start_date)
            ...
            cursor.execute("""
                ...
                BEGIN
                    ...
                    IF NOT EXISTS (
                        ...
                        SIGNAL SQLSTATE '45000'
                        SET MESSAGE_TEXT = 'Player is not a valid member of the team at the match date.';
                    END IF;
                END;
            """)
            cursor.fetchall()  # Clear any results
            ...
            cursor.execute("""
                CREATE TRIGGER prevent_player_time_conflict
                BEFORE INSERT ON match_players
                FOR EACH ROW
                BEGIN
                    DECLARE v_match_date VARCHAR(10);
                    ...
                    SELECT date, time_slot INTO v_match_date, v_time_slot
                    FROM matches
                    WHERE match_id = NEW.match_id;
                    ...
                    IF EXISTS (
                        ...
                        AND m.date = v_match_date
                        AND (
                            v_time_slot BETWEEN m.time_slot AND m.time_slot + 1
                            OR m.time_slot BETWEEN v_time_slot AND v_time_slot + 1
                        )
                    ) THEN
                        SIGNAL SQLSTATE '45000'
                        SET MESSAGE_TEXT = 'Player has a time conflict with another match.';
                    END IF;
                END;
            """)
            cursor.fetchall()  # Clear any results
            ...
            cursor.execute("""
                CREATE TRIGGER validate_team_registered_to_tournament
                BEFORE INSERT ON match_players
                FOR EACH ROW
                BEGIN
                    ...
                    IF NOT EXISTS (
                        ...
                        SIGNAL SQLSTATE '45000'
                        SET MESSAGE_TEXT = 'Team is not registered in the tournament.';
                    END IF;
                END;
            """)
            cursor.fetchall()  # Clear any results
...
if __name__ == "__main__":
    ...
    excel_file = "chess_db_app/ChessDB_updated.xlsx"
    try:
