...
@app.route('/rename_hall/<int:hall_id>', methods=['POST'])
@login_required
@role_required(['db_manager'])
def rename_hall(hall_id):
    """Handle hall renaming by database manager"""
    try:
        new_name = request.form.get('new_name')
        if not new_name:
            flash('New hall name is required', 'error')
            return redirect(url_for('dashboard', role='db_manager'))
